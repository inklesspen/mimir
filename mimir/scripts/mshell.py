...
from montague import load_app, load_logging_config
from montague.loadwsgi import Loader

from pyramid.scripts.common import parse_vars

def bootstrap(config_uri, request=None, options=None):
    path, section = _getpathsec(config_uri, None)
    app = load_app(path, section)
    env = prepare(request)
    env['app'] = app
    return env
...
class PShellCommand(object):
    ...
    def run(self, shell=None):
        ...
        dictConfig(load_logging_config(config_file, config_uri.split('#', 1)[1]))
        # self.pshell_file_config(config_file)  # TODO

...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("ongoing", "abandoned", "completed", native_enum=False),
            nullable=False,
        ),
    ...
    op.create_index(
        op.f("ix_writeups_author_slug"), "writeups", ["author_slug"], unique=False
    )
    op.create_index(
        op.f("ix_writeups_writeup_slug"), "writeups", ["writeup_slug"], unique=False
    )
    op.create_table(
        "thread_pages",
        sa.Column("id", sa.Integer(), nullable=False),

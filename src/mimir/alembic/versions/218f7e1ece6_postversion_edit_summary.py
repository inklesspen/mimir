...
def upgrade():
    op.add_column(
        ...
        sa.Column("edit_summary", sa.Unicode(length=200), nullable=True),
    )
    ...
    op.alter_column(
        "writeup_post_versions", "extracted_at", new_column_name="created_at"
    )
...
def downgrade():
    ...
    op.alter_column(
        "writeup_post_versions", "created_at", new_column_name="extracted_at"
    )

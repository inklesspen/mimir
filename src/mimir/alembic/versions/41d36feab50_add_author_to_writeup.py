...
def upgrade():
    op.add_column(
        "writeups", sa.Column("author", sa.Unicode(length=100), nullable=True)
    )
    t_w = sa.table(
        "writeups", sa.column("id", sa.Integer), sa.column("author", sa.String),
    )
    t_wp = sa.table(
        "writeup_posts",
    ...
    stmt = sa.select([t_wp.c.author]).where(
        sa.and_(t_wp.c.writeup_id == t_w.c.id, t_wp.c.index == 1)
    )
